...
另外，这套流程是在合成线程中完成的，合成线程是独立于主线程的另一条线程，这样哪怕主线程执行了很耗时的 JS 任务，也不会阻塞到合成线程的工作。由于某些 CSS 动画只需要合成线程便可实现，所以善用这些动画就可以减轻主线程的负荷，同时也能得到流畅的体验。
...
接下来，主线程会将图层树与绘制指令发送给合成线程，合成线程是渲染进程内的另一个线程，它负责图层分块、光栅化、合成。

由于一个图层可能会有整个页面那么大，为了更快的光栅化，合成线程首先会将图层分割成小块，然后再将图块发送给光栅线程。

另外，合成线程还会设置光栅化线程的优先级，这样就可以优先光栅化在 viewport 内（或在 viewport 附近）的图块。并且，同一个图块还有多种版本，每种版本的区别是分辨率不同，这是为了应对浏览器的放大缩小操作。
...
当光栅化线程生成了足够显示页面的位图后，合成线程就会收集这些位图的信息，这个信息是指位图在 GPU 内存中的地址，以及位图要放置在页面的哪个位置。Chrome 将这些信息称为 `draw quads`，一个位图对应一份 `draw quads`。

然后合成线程会将这些 `draw quads` 的集合通过 IPC 发送给浏览器进程，Chrome 将这些 `draw quads` 的集合称为 `Compositor frame`，它不是真正的位图，而是对图块位图的映射。
...
当页面发生滚动或动画时，合成线程就会创建另一个新的 `Compositor frame` 来发送给 GPU，最后 GPU 就会更新页面。

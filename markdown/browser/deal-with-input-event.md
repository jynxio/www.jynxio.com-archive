...
如果事件监听器或默认动作修改了页面的样式，那么浏览器就会在渲染进程的主线程清空掉所有任务之后再渲染出新的画面。如果主线程上的任务非常耗时，那么在新画面被渲染出来之前，页面会被表现为假死状态，即用户无法与页面进行任何交互。

> 你需要学习浏览器的事件循环，才能真正理解上一段话。

唯一的例外是，开发者可以通过 [Passive event listeners](https://github.com/WICG/EventListenerOptions/blob/gh-pages/explainer.md) 特性来让页面的滚动不受主线程的阻塞。
